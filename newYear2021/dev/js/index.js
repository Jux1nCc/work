var newYear = {
      ...
      if (guardItem.length > 0) {
        ...
        for (var i = 0; i < guardItem.length; i++) {
          var str =
            ...
            guardItem[i].nickName +
            '</div>' +
            '</div>' +
            '<div class="gw-name gw-boyName flexbox">' +
            '<div class="gw-name-item">' +
            guardItem[i].opNickName +
            '</div>' +
            '</div>' +
  ...
  mountedGuard: function (data) {
    if (data.ranks) {
      ...
      if (ranks.length > 0) {
        ...
        if (ranks.length >= 15) {
          $('.gi-wrap ul').append(
            '<li class="look-more popup" onclick="lookMore(this)">点击查看更多<img src="./images/more.png"></li>'
          )
        }
      }
    }
  ...
  shake: function (ele) { // '.is-next .info-fir span'
