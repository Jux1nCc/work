...
var WSCZGBBusiComp = {
   ...
   // var  dcValue = value_a.substring(0, value_a.length - 1);
    ...
    if($("#paytype").val() && $("#paytype").val() =='11'){
		...
		"	onclick=\"javascript:lijichongzhi();\" " +
...
//        WSCZGBBusiComp.showMoney();
    ...
    selectPaytypeSecond: function (obj, num) {
    	 if('3'==num){
         	...
         	$("#imgFor3p").attr("src","files/Alipay_p.png");
         	$("#imgFor11p").attr("src","files/wechat_n.png");
         	var html="<div class=\"f_cz_rig\" collect=\"CZSY_23\" nextarea=\"true\" " +
									"	style=\"width: 100%\" nextpage=\"true\" id=\"czBtn\" " +
									"	businessid=\"E051709351477646480bid\" businessname=\"bid\" " +
									"	onclick=\" lijichongzhi() \" "+
									"	touchvid=\"T051709392996884432\" touchcode=\"CZSY_23\" " +
									"	collectready=\"true\">立即充值</div>";
         	$("#payTypeButton").html(html);
         ...
         if('11'==num){
         	...
         	$("#imgFor3p").attr("src","files/Alipay_n.png");

         	$("#imgFor11p").attr("src","files/wechat_p.png");
...
//        }
        ...
        if ("" == targetPhone || targetPhone == null) {
        	alert("请先绑定账号");
        }
        ...
        var isFrom=getURLParameter("isFrom");
...
function Bind(){
	...
	if(account && account.length>0){
		$.ajax({
			...
			type : "POST",
			data : {"customerId":account},
			dataType : "json",
			success : function(data) {
				if(data.status=="ok"){
					if(data.hasCoupon && data.hasCoupon == 1 && !isWeiXin()){
						$(".yhq-empty-list").html("");
						$(".xzCardroll").show();
						var couponInfo = data.couponInfo;
						var html = "<li>原价支付<input type=\"radio\" checked  name=\"coupon\" value=\"0\" onclick=\"selectCouponId(this)\" title=\"\" /></li>";
						$.each(couponInfo,function(index,value){
							html += "<li>"+value.couponType+"（<span>"+value.discountsType+"</span>）<input type=\"radio\" onclick=\"selectCouponId(this)\" name=\"coupon\" value=\""+value.cashInAmt+"\" title=\""+value.couponId+"\" /></li>";
						});
						$(".yhq-empty-list").html(html);
						$("#couponDiv").show();
					}
					$(".imgtest").show();
					$("#inputDiv").hide();
					$("#customerInfoForName").text("(兔聊号: "+data.customerId);
					$("#customerInfoForNickName").text("昵称: "+data.nickName+")");
					$("#customerInfoForPhoto").attr("src",data.photo);
					$("#mobile").val(data.customerId);
					$("#boundBtn").html("<img id=\"bandBt\" style=\" margin: 5px 0 5px 2.5%; width: 98.88px; height: 39.552px; top: 20.6px; left: 297.739px;\" src=\"files/changeBt.png\" onclick=\"changeBt()\">");
					isBaund = 1;
					cid = data.customerId;
				}else{
					alert("账号不存在，请重新输入。");
...
function couponClick(){
	$("#couponDiv").hide();
//	selectCouponType = 100;
	var labels = $("li[name='cashInLabel']");
	var index2 = 1;
	if(selectCouponType > 0){
		var removeIndex = 1;
		$.each(labels,function(index,value){
			if($(value).val() < selectCouponType){
				$(value).css("opacity", 0.5);
				var lab = $(value).children("label");
				removeIndex ++;
			}else{
				if(index2 == 1){
					var lab = $("radio[name='zffs']");
					$(lab).attr("checked","checked");
				}
				index2 ++;
			}
		});

		var checkbb = $("span[name='spanAfter']");
//		alert(removeIndex);
		$.each(checkbb,function(index,value){
		...
		var tt = $("input[name='zffs']");
		$.each(tt,function(index,value){
			if(index == removeIndex -1){
				$(value).attr("checked","checked");
				WSCZGBBusiComp.abs(this,$(value).val());
			}
...
var couponId="";
var selectCouponType = 0 ;
function selectCouponId(obj){
	if($(obj).is(":checked")){
		selectCouponType = $(obj).val();
		couponId = $(obj).attr("title");
		if(selectCouponType>0){
			$("#couponName").text("卡券支付>");
		}else{
			$("#couponName").text("原价支付>");
		}
	}
}
...
function changeBt(){
	...
	$(".imgtest").hide();
	$("#inputDiv").show();
	$("#customerInfoForName").text("");
	$("#customerInfoForNickName").text("");
	$("#customerInfoForPhoto").attr("src","");
	cid = "";
}
...
var isBaund = $("#isBound").val();
function isBound(){

}

function lijichongzhi(){
	var paytype = $("#paytype").val();
	if(isWeiXin() && paytype==3){
		//微信内支付宝充值，此时提示跳转浏览器
	...
	if(!isWeiXin() && paytype==3){
		var cate = $('#czBtn').attr('first');
		if(cate==undefined)
			WSCZGBBusiComp.zhifuOtherPay();
		else
			WSCZGBBusiComp.zhifuFastPay();
		if(typeof(_tag)!='undefined')
		{
			_tag.dcsMultiTrack('WT.nv','WSCZ', 'WT.event','WSCZ_LJCZ_m');
		}
	}
}
...
function getOpenInfo(){
	var amount = $("#amount").val();
	$.ajax({
		url : "../../view/testPay/payOfWeixin/", // 新版JSAPI_ForVer3.X
		type : "POST",
		data : {"mycode":mycode,"money":amount,"customerID":cid},
		dataType : "json",
		success : function(data) {
			mycode = "";
			wxPay(data);
		...
		if (res.err_msg == "get_brand_wcpay_request:ok") {
			$.ajax({
				...
				success : function(data) {
					if(data.status=="ok"){
//						alert("充值成功!");
						var bObj=document.getElementById("alertbg");
						bObj.onclick= jumpNewUrl;
						$('#alertbg').css('display', 'block');
						$('#alert1').css('display', 'block');
						$('#alertTitle4').css('display', 'block');
						$('#alertTitle5').css('display', 'block');
					}
				},
				error : function(x, h, e) {
...
$(function () {
	...
	$('#iconImg').width(121 * sc);
	$('#iconImg').height(120 * sc);
	$('#iconImg').css('left', 30 * sc);
	$('#iconImg').css('top', 50 * sc);
	$('#iconImg').css('border-radius', 30);


	$('#disbandBt').width(180 * sc);
	$('#disbandBt').height(72 * sc);
	$('#disbandBt').css('top', 77 * sc);
	$('#disbandBt').css('left', 542 * sc);

	$('#alertTitle').css('font-size', 22 * sc);
	$('#alertTitle').css('top', 424 * sc);
	$('#alertTitle').css('left', 273 * sc);
	$('#alertTitle').width(200 * sc);
	$('#alertTitle').css('line-height', 50 * sc + "px");

	$('#alertTitle2').css('font-size', 22 * sc);
	$('#alertTitle2').width(197 * sc);
	$('#alertTitle2').css('line-height', 40 * sc + "px");
	$('#alertTitle2').css('top', 427 * sc);
	$('#alertTitle2').css('left', 274 * sc);

	$('#alertTitle3').css('font-size', 22 * sc);
	$('#alertTitle3').css('top', 605 * sc);
	$('#alertTitle3').css('left', 72 * sc);
	$('#alertTitle3').width(606 * sc);
	$('#alertTitle3').height(90 * sc);
	$('#alertTitle3').css('line-height', 91 * sc + "px");

	$('#alertTitle4').css('font-size', 22 * sc);
	$('#alertTitle4').width(197 * sc);
	$('#alertTitle4').css('line-height', 40 * sc + "px");
	$('#alertTitle4').css('top', 427 * sc);
	$('#alertTitle4').css('left', 274 * sc);

	$('#alertTitle5').css('font-size', 22 * sc);
	$('#alertTitle5').css('top', 605 * sc);
	$('#alertTitle5').css('left', 72 * sc);
	$('#alertTitle5').width(606 * sc);
	$('#alertTitle5').height(90 * sc);
	$('#alertTitle5').css('line-height', 91 * sc + "px");

	$('#alert1').width(606 * sc);
	$('#alert1').height(350 * sc);
	$('#alert1').css('left', (clientWidth * 0.5) - (303 * sc));
	$('#alert1').css('top', 345 * sc);
...
function jumpNewUrl(){
	window.location.href="https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxd7555f83fe7e0627&redirect_uri=https%3a%2f%2fwxgzh.folkcam.cn%2fview%2fweixinopen%2fWAPRECHAGE.html&response_type=code&scope=snsapi_base&state=123#wechat_redirect";
}
