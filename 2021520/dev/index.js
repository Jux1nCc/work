...
function lookRanksData(that, type) {
  ...
  for (var i = 0; i <= 5; i++) {
    if (!$($('.rank-item-date li')[i]).hasClass('later')) {
      $($('.rank-item-date li')[i]).addClass('ago')
    }
  }
...
function changePageNav(type, that) {
  ...
  if (type == 2) {
    ...
    // $('.main-rule').removeClass('popup').siblings('.main-award').removeClass('popup')
