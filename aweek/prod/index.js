var sevenAct = {
  ...
  init: function () {
    ...
    if (window.location.href.split('?')[2] && window.location.href.indexOf('hang') != -1) {
      sevenAct.isAlert()
    }
    sevenAct.diffAPP()
    sevenAct.diffsystem()
  ...
  diffAPP: function () {
    if (sevenAct.appType == 1) {
      ...
      $('.wrap>p').css('background-image', 'url("image/rule_tuliao.png")')
    } else {
      $('.appName').text('觅伊')
      $('.appUnit').html('直播<br>收益（积分）')
      $('.wrap>p').css('background-image', 'url("image/rule_miyi.png")')
    }
  },
  diffsystem: function () {
    if (sevenAct.android) {
...
function toRule() {
  window.location.href = "./rule.html?" + window.location.href.split('?')[1] + '&v=001'
}
function toIndex() {
  window.history.go(-1)
}
