...
function lookMore(that) {
  ...
  var parmas = {
    ...
    page: singles.page,
    pageSize: singles.ranks.length > 479 ? 20 : singles.pageSize
  }
  $.ajax({
    type: 'GET',
    url: $config.toUrl(singles.env, '/act/eleven20/getCallRank'),
    ...
    success: function (res) {
      ...
      if (res.result) {
        ...
        if(singles.ranks.length > 499 && res.data.ranks.length == 20){
          $('.list-wrap').append('<li class="look-more reset-style" >榜单仅展示前500名哦</li>')
          $($('.look-more')[($('.look-more').length-1)]).removeClass('popup')
        }else {
          $('.list-wrap').append('<li class="look-more" onclick="lookMore(this)">点击查看更多>></li>')
          $($('.look-more')[($('.look-more').length-1)]).removeClass('popup')
        }
