var wuyi = {
  ...
  env: 'demo', // demo
  customerId: $config.getQueryStringByName("customerId"), // 用户ID
  isFrom: $config.getQueryStringByName("isFrom"), // 网页来源
  token: $config.getQueryStringByName("token"), // 用户token
  appType: $config.getQueryStringByName("appType").slice(0, 1), // APP类型 1-兔聊 2-觅伊
  sex: 0, // 性别
  beginDate: null,  // 开始日期
  ...
  init: function () {
    if (wuyi.isFrom == 'live') {
      $('.layui-layer.layui-layer-page.EnterName').css('cssText', 'top: 0 !important')
      $('.layui-layer.layui-layer-page.ChangeName').css('cssText', 'top: 0 !important')
      $('.layui-layer.layui-layer-page.DoInvite').css('cssText', 'top: 0 !important')
      $('.layui-layer.layui-layer-page.AcceptInviteOne').css('cssText', 'top: 0 !important')
      $('.layui-layer.layui-layer-page.AcceptInviteMulti').css('cssText', 'top: 0 !important')
    }
    wuyi.created()  // 请求初始数据
    wuyi.appMounted() // app 的区别
  },
  created: function () {
    var parmas = {
    ...
    $.ajax({
      ...
      success: function (res) {
        // if (!wuyi.appBool) {  // 是否在App外打开
        //   if (wuyi.appType == 1) {
        //     $('#openOutApp').css('background-image', 'url(images/dia_open_app_rabbit.png)')
        //   }
        //   dialogCommInit($('#openOutApp'), 'openOutApp')
        //   return
        // } else if (!wuyi.isLogin) { // 否
        //   dialogCommInit($('#isLogin'), 'isLogin')
        //   return
        // }
        if (res.result) {
  ...
  mountedRecommendUser: function (data, treeId) {
    ...
    $('.search-result-btn').attr('treeId', treeId)
    if (data.length <= 0) {
  ...
  showGradeTip: function (data) {
    ...
    dialogCommInit($('#GradeTip'), 'GradeTip')
  },
...
function beginGrow() {
  ...
  //   return
  // }
  dialogCommInit($('#EnterName'), 'EnterName')
  changeDiaTop()
}
...
function pendingInvite() {
  if (wuyi.inviteTrees.length == 0) {
    ...
    if (wuyi.inviteTrees.length > 1) {
      ...
      dialogCommInit($('#AcceptInviteMulti'), 'AcceptInviteMulti')
      changeDiaTop()
    } else {
      ...
      dialogCommInit($('#AcceptInviteOne'), 'AcceptInviteOne')
      changeDiaTop()
    }
  }
}
...
function dialogCommInit(contain, className) {
  layer.closeAll()
  layer.open({
    type: 1,
    shadeClose: false,
...
function inviteBoy(treeId) {
  ...
  dialogCommInit($('#DoInvite'), 'DoInvite')
  changeDiaTop()
}
// 男方单个接受邀请
...
function refreshRecommend(that) {
  ...
  //   usedIds = usedIds + $($(eleArr[i]).children('.search-result-btn')).attr('customerId') + ','
  //   treeId = $($(eleArr[i]).children('.search-result-btn')).attr('treeId')
  // }
  var parmas = {
    customerId: wuyi.customerId,
    token: wuyi.token,
...
function toShare() {
  ...
  var img = "http://miyiurl.rklive888.com/labour/dev/images/header_bg.png",
    title = '劳动节合种爱情树',
...
function changeTreeName(that) {
  ...
  dialogCommInit($('#ChangeName'), 'ChangeName')
  changeDiaTop()
}
