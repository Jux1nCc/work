var spring = {
  ...
  appType: $config.getQueryStringByName("appType").split('?')[0] || 2,
  photoUrlObj: [],
  customerIdObj: [],
  nickNameObj: [],
  hasPosition: [],
    ...
    for (let j = 0; j < spring.CHANNEL_COUNT; j++) {
      ...
      for (let i = 0; i < spring.MAX_DM_COUNT; i++) {
        ...
        dom.addEventListener('transitionend', function () {
          dom.style.transform = `translateX(${(-dom.children[0].children[0].clientWidth) / 20}rem)`;
          setTimeout(function(){
            dom.className = 'right';
  ...
  shootDanmu: function (dom, text, channel, photoUrl, customerId, nickName) {
    var str =
      ...
      '</div>'
    dom.innerHTML = str
    dom.style.transform = `translateX(${(-dom.children[0].children[0].clientWidth) / 30}rem)`;
    dom.className = 'left';
    ...
    }, dom.children[0].children[0].clientWidth * 10 + 2000);
  },
  getMessage: function () {
    var parmas = {
